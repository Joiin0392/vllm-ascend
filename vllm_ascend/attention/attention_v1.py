...
class AscendAttentionBackendImpl(AttentionImpl):
    ...
    def _fallback_sdpa(
        ...
        # actual_seq_lengths_q is cumulative, compute individual query lengths
        query_lens = []
        prev = 0
        ...
        # actual_seq_lengths_kv can be cumulative (in PrefillNoCache) or individual (in other states)
        if attn_metadata.attn_state == AscendAttentionState.PrefillNoCache:
            if self.attn_type == AttentionType.ENCODER_DECODER:
                kv_lens = []
                prev = 0
        ...
        for i in range(len(query_lens)):
            ...
            # Handle query size mismatch due to Chunked Prefill + Speculative Decoding
            ...
            #  Use += instead of assignment for proper accumulation
            prev_q += cur_q
            prev_kv += cur_kv
        
        attn_output = torch.cat(outputs, dim=0)
        
        # parallel:Use actual output size instead of metadata num_tokens
    ...
    def forward_fused_infer_attention(
        ...
        # Fix: Check head_size BEFORE _EXTRA_CTX.capturing to avoid graph mode issues
        # But we need to get parameters first
